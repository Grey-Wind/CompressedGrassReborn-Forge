...
public class DoubleCompressedGrassTradingTrade {
	@SubscribeEvent
	public static void registerTrades(VillagerTradesEvent event) {
		Int2ObjectMap<List<VillagerTrades.ITrade>> trades = event.getTrades();
		if (event.getType() == VillagerProfession.FARMER) {
			trades.get(1).add(
					new BasicTrade(new ItemStack(GrassIngotItem.block, (int) (10)), new ItemStack(DoubleCompressedGrassItem.block), 10, 5, 0.05f));
		}
	}
}
