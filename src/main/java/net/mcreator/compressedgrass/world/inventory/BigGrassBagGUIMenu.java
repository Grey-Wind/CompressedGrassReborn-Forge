...
public class BigGrassBagGUIMenu extends AbstractContainerMenu implements Supplier<Map<Integer, Slot>> {
	...
	public BigGrassBagGUIMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
		super(CompressedGrassModMenus.BIG_GRASS_BAG_GUI.get(), id);
	...
	public ItemStack quickMoveStack(Player playerIn, int index) {
		...
		if (slot != null && slot.hasItem()) {
			...
			if (index < 115) {
				if (!this.moveItemStackTo(itemstack1, 115, this.slots.size(), true))
					return ItemStack.EMPTY;
				slot.onQuickCraft(itemstack1, itemstack);
			} else if (!this.moveItemStackTo(itemstack1, 0, 115, false)) {
				if (index < 115 + 27) {
					if (!this.moveItemStackTo(itemstack1, 115 + 27, this.slots.size(), true))
						return ItemStack.EMPTY;
				} else {
					if (!this.moveItemStackTo(itemstack1, 115, 115 + 27, false))
						return ItemStack.EMPTY;
				}
				return ItemStack.EMPTY;
			}
